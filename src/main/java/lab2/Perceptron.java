...
public class Perceptron {
  ...
  double[] hiddenN;
  double[] hiddenNF;//после применения ф сигмоиды
  List<Data> data;
  int round;
  int outer;

  public Perceptron(List<Data> data) {
    this.data = data;
    round = (int) (data.size() * 0.8);
    System.out.println(data.size());
    inputN = new double[2];
    hiddenN = new double[2];
    hiddenNF = new double[2];
...
//      System.out.println("y=" + y);
    ...
    for (int i = 0; i < wih.length; i++) {
      for (int j = 0; j < wih.length; j++) {
        System.out.println("wih[" + i + "][" + j + "]=" + wih[i][j]);
      }
    }
  ...
  public void cy() {
    ...
    for (int i = 0; i < hiddenN.length; i++) {
      ...
      hiddenNF[i] = sigmoid(hiddenN[i]);//полсе применения функции сигмоиды
...
//        inputN[1] = data.get(p).closeSum;
        ...
        for (int i = 0; i < inputN.length; i++) {
          for (int j = 0; j < hiddenN.length; j++) {
            //сумма(скорость обучения * ошибка с предыдущего слоя(err[i]) * значения входа * производную от текущего)
            wih[i][j] += learningRate * err[j] * revSigmoid(hiddenN[i]) * inputN[i];
          }
        }
        for (int i = 0; i < hiddenN.length; i++)
          why[i] += learningRate * lEr * hiddenNF[i] * revSigmoid(hiddenN[i]);
